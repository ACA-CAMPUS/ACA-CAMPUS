...
function renderSafetyLibrary(){return `<section class="safety-library" aria-labelledby="safetyLibraryTitle"><div class="safety-hero"><div><span class="eyebrow">Biblioteca de consulta rápida</span><h2 id="safetyLibraryTitle">Fichas Técnicas y de Seguridad</h2><p>Consultá procedimientos, controles y recomendaciones operativas para distintos sectores de la estación.</p></div><span class="safety-hero-icon">${safetyIcon()}</span></div><div class="safety-toolbar"><label class="safety-search"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="7"></circle><path d="m20 20-4-4"></path></svg><span class="sr-only">Buscar fichas técnicas y de seguridad</span><input id="safetySearch" type="search" placeholder="Buscar ficha, sector o tema..." autocomplete="off" value="${safetySearchQuery.replace(/"/g,'&quot;')}"></label><p id="safetyCount" class="safety-count">${safetySheets.length} fichas disponibles</p></div><div class="safety-filters" id="safetyFilters" aria-label="Filtrar fichas por sector o tema">${safetyCategories.map(category=>`<button class="filter ${category===activeSafetyFilter?'active':''}" data-safety-filter="${category}" aria-pressed="${category===activeSafetyFilter}">${category}</button>`).join('')}</div><div id="safetyList">${renderSafetyCards(filteredSafetySheets())}</div></section>`}
function renderToolsPage(){return `${head('Caja de Herramientas del Capacitador','Recursos de consulta y aplicación para acompañar la formación y la operación cotidiana.')}<section class="facilitation-tools" aria-labelledby="facilitationTitle"><div class="section-head"><div><span class="eyebrow">Dinámicas de aprendizaje</span><h2 id="facilitationTitle">Herramientas para facilitar</h2></div></div><div class="tools-grid">${tools.map(t=>`<article class="card tool-card lift"><div class="tool-icon">◇</div><h3>${t[0]}</h3><p>${t[1]}</p><div class="meta"><span>◷ ${t[2]}</span><span class="badge">${t[3]}</span></div><div class="card-actions"><button class="btn" data-resource="${t[0]}">Ver recurso</button></div></article>`).join('')}</div></section>${renderSafetyLibrary()}`}
...
document.addEventListener('click',event=>{const filter=event.target.closest('[data-safety-filter]');if(filter){activeSafetyFilter=filter.dataset.safetyFilter;document.querySelectorAll('[data-safety-filter]').forEach(button=>{const active=button===filter;button.classList.toggle('active',active);button.setAttribute('aria-pressed',String(active))});updateSafetyLibrary()}const sheetButton=event.target.closest('[data-safety-sheet]');if(sheetButton)openSafetySheet(sheetButton.dataset.safetySheet)});
